...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture(r"Drone_Test_Set/Simulation_diagonal.mp4")
    if(cap.isOpened() == False):
            print("Error opening video")
            quit()
    ret, frame = cap.read()

    particles = Particle_Filter(no_particles=500,world_size=(frame.shape[1], frame.shape[0]))          # Initialise Particle Filter
    ...
    print("Average time to process: %.3f seconds"%(total_time/(frame_no-2)))
    cap.release()
